...
$('.last-products__control-btn').click(function(){
    var _this = $(this),
        controls = $('.last-products__control-item'),
        control = _this.parent()
...
/**
 * *** hasClass ***
 * Функція повертає true або false якщо клас був знайдений чи ні () у 
 * переданому массиві класів
 *
 * @param {array} array - collection classList
 * @param {string} searchClass - class that you need to find
 *
 * @return {boolean}
 */
function hasClass(array: string[], searchClass: string){
...
(function slideExcellenceAccordion() {
    ...
    var items = document.querySelectorAll('.excellence__item'),
        count: number = items.length;

    for (let index = 0; index < count; index++) {
        items[index].addEventListener('click', function() {
            var siblings = [],
                el = this.parentNode.firstElementChild;
 ...
 * Typed
 */
$(function(){var options1={strings:[$('#text_1').html()],typeSpeed:50}
...
$(".sl_1").typed(options1);$(".icon-right-open-big").click(function(){$(".sl_1").typed(options1);$(".sl_2").typed(options2);$(".sl_3").typed(options3);$(".sl_4").typed(options4);$(".sl_5").typed(options5);});});});function TypedText(){$("#sl_1").typed({strings:['<span class="slider-item-text orange"> Случилась неожиданная поломка?..</span><br /><span class="slider-item-text"> Решим проблему любого водителя!</span>'],typeSpeed:50,loop:true});$("#sl_2").typed({strings:['<span class="slider-item-text orange"> Даем гарантию оригинальности детали.</span><br /><span class="slider-item-text"> Полная диагностика. Проверка двигателя.</span>'],typeSpeed:50,loop:true});$("#sl_3").typed({strings:['<span class="slider-item-text orange"> Сломались?</span><br /><span class="slider-item-text"> Мы найдём для Вас решение!</span>'],typeSpeed:50,loop:true});}
